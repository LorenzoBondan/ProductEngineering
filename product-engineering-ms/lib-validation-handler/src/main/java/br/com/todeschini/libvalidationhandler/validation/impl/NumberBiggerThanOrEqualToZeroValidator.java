...
public class NumberBiggerThanOrEqualToZeroValidator implements Validator<Number> {

    @Override
    public ValidationResult validate(Number s) {
        ValidationResult validationResult = new ValidationResult(true);
        if(s != null && new BigDecimal(s.toString()).compareTo(BigDecimal.ZERO) < 0){
            validationResult = new ValidationResult(false, "Number must be bigger than or equal to zero");
        }
