...
const DrawerPullModal: React.FC<DrawerPullModalProps> = ({ drawerPull, isOpen, isEditing, onClose, onDeleteOrEdit }) => {

  const [errorMessage, setErrorMessage] = useState<string>('');
  const { register, handleSubmit, formState: { errors }, setValue } = useForm<DDrawerPull>();
  const [dateTime, setDateTime] = useState<Date | null>(null);

  // load inputs

  useEffect(() => {
    if(isEditing && drawerPull){
        drawerPullService.findById(drawerPull.code)
        .then((response) => {
            const fetchedDrawerPull = response.data as DDrawerPull;

            setValue('code', fetchedDrawerPull.code);
            setValue('description', fetchedDrawerPull.description);
            setValue('measure1', fetchedDrawerPull.measure1);
  ...
  }, [isEditing, drawerPull, setValue]);

  // insert / update method

  const insertOrUpdate = (formData: DDrawerPull) => {
    if (dateTime !== null) {
      formData.implementation = dateTime;
    }
  
    const serviceFunction = isEditing ? drawerPullService.update : drawerPullService.insert;
...
export default DrawerPullModal;
