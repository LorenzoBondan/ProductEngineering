...
export default function PaintingForm() {
    ...
    const isEditing = params.paintingId !== 'create';
    ...
    useEffect(() => {
        if (isEditing) {
            pinturaService.pesquisarPorId(Number(params.paintingId))
                .then(response => {
    ...
    function handleSubmit(event: any) {
        ...
        if (isEditing) {
            requestBody.codigo = Number(params.paintingId);
        }

        const request = isEditing
            ? pinturaService.atualizar(requestBody)
            : pinturaService.criar(requestBody);
