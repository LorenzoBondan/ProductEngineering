...
export default function MoldingForm() {
    ...
    const isEditing = params.moldingId !== 'create';
    ...
    useEffect(() => {
        if (isEditing) {
            bagueteService.pesquisarPorId(Number(params.moldingId))
                .then(response => {
    ...
    function handleSubmit(event: any) {
        ...
        if (isEditing) {
            requestBody.codigo = Number(params.moldingId);
        }

        const request = isEditing
            ? bagueteService.atualizar(requestBody)
            : bagueteService.criar(requestBody);
