...
export default function PlasticForm() {
    ...
    const isEditing = params.plasticId !== 'create';
    ...
    useEffect(() => {
        if (isEditing) {
            plasticoService.pesquisarPorId(Number(params.plasticId))
                .then(response => {
    ...
    function handleSubmit(event: any) {
        ...
        if (isEditing) {
            requestBody.codigo = Number(params.plasticId);
        }

        const request = isEditing
            ? plasticoService.atualizar(requestBody)
            : plasticoService.criar(requestBody);
