...
export default function PolyethyleneForm() {
    ...
    const isEditing = params.polyethyleneId !== 'create';
    ...
    useEffect(() => {
        if (isEditing) {
            polietilenoService.pesquisarPorId(Number(params.polyethyleneId))
                .then(response => {
    ...
    function handleSubmit(event: any) {
        ...
        if (isEditing) {
            requestBody.codigo = Number(params.polyethyleneId);
        }

        const request = isEditing
            ? polietilenoService.atualizar(requestBody)
            : polietilenoService.criar(requestBody);
