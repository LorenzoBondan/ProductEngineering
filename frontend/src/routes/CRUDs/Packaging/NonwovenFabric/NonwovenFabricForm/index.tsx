...
export default function NonwovenFabricForm() {

    const params = useParams();

    const navigate = useNavigate();

    const isEditing = params.nonwovenFabricId !== 'create';
    ...
    useEffect(() => {
        if (isEditing) {
            tntService.pesquisarPorId(Number(params.nonwovenFabricId))
                .then(response => {
    ...
    function handleSubmit(event: any) {
        ...
        if (isEditing) {
            requestBody.codigo = Number(params.nonwovenFabricId);
        }

        const request = isEditing
            ? tntService.atualizar(requestBody)
            : tntService.criar(requestBody);
