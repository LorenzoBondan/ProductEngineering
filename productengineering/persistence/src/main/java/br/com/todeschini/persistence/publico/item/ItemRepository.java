...
public interface ItemRepository extends JpaRepository<Item, Long> {
    ...
    Collection<Item> findByDescription(String description); // usado para verificar registro duplicado

    @Query("SELECT c FROM Item c WHERE c.status = 'ACTIVE' OR (:id IS NOT NULL AND c.code = :id)")
    List<Item> findAllActiveAndCurrentOne(@Param("id") Long id);
    ...
    @Query(nativeQuery = true, value = """
            SELECT created_by FROM tb_item WHERE code = :id
            """)
    String findCreatedBy(@Param("id") Long id);

    @Query(nativeQuery = true, value = """
            SELECT creation_date FROM tb_item WHERE code = :id
            """)
