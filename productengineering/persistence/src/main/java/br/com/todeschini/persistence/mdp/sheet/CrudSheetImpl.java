...
@Component
public class CrudSheetImpl implements CrudSheet {

    private final SheetRepository repository;
    private final SheetDomainToEntityAdapter adapter;
    private final EntityService entityService;

    public CrudSheetImpl(SheetRepository repository, SheetDomainToEntityAdapter adapter, EntityService entityService) {
    ...
    public Collection<? extends DSheet> findByDescription(String description) {
        return repository.findByDescription(description).stream().map(adapter::toDomain).toList();
    }

    @Override
    @Transactional(readOnly = true)
    public List<DSheet> findAllActiveAndCurrentOne(Long obj) {
    ...
    @Override
    @Transactional
    public DSheet insert(DSheet obj) {
    ...
    @Override
    @Transactional
    public DSheet update(Long id, DSheet obj) {
        if(!repository.existsById(id)){
            throw new ResourceNotFoundException("Código não encontrado: " + id);
        }
        Sheet entity = adapter.toEntity(obj);
    ...
    public void inactivate(Long id) {
        Sheet entity = repository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Código não encontrado: " + id));
    ...
    private void setCreationProperties(Sheet obj){
