...
@Component
public class CrudBinarioImpl implements CrudBinario {

    private final BinarioRepository repository;
    private final BinarioQueryRepository queryRepository;
    private final BinarioDomainToEntityAdapter adapter;
    private final EntityService entityService;
    private final PageRequestUtils pageRequestUtils;
    private final HistoryService historyService;
    private final SituacaoFilter<Binario> situacaoFilter;
    private final AuditoriaService auditoriaService;

    public CrudBinarioImpl(BinarioRepository repository, BinarioQueryRepository queryRepository, BinarioDomainToEntityAdapter adapter,
                           EntityService entityService, PageRequestUtils pageRequestUtils, HistoryService historyService, SituacaoFilter<Binario> situacaoFilter, AuditoriaService auditoriaService) {
    ...
    @Override
    public Paged<DBinario> buscarTodos(PageableRequest request) {
        ...
        return Optional.of(queryRepository.findAll(specification, pageRequestUtils.toPage(request)))
                .map(r -> new PagedBuilder<DBinario>()
    ...
    public List<DHistory<DBinario>> buscarHistorico(Integer id) {
        return historyService.getHistoryEntityByRecord(Binario.class, "tb_binario", id.toString(), AttributeMappings.BINARIO.getMappings()).stream()
    ...
    public List<String> buscarAtributosEditaveisEmLote() {
        return entityService.obterAtributosEditaveis(DBinario.class);
    }

    @Override
    public DBinario inserir(DBinario obj) {
        entityService.verifyDependenciesStatus(adapter.toEntity(obj));
        return adapter.toDomain(repository.save(adapter.toEntity(obj)));
    }

    @Override
    public DBinario atualizar(DBinario obj) {
        if(!repository.existsById(obj.getCodigo())){
            throw new ResourceNotFoundException("Código não encontrado: " + obj.getCodigo());
        }
        Binario entity = adapter.toEntity(obj);
    ...
    @Override
    public List<DBinario> atualizarEmLote(List<DBinario> list) {
        return list;
    }

    @Override
    public DBinario substituirPorVersaoAntiga(Integer id, Integer versionId) {
        DHistory<Binario> antiga = historyService.getHistoryEntityByRecord(Binario.class, "tb_binario", id.toString(), AttributeMappings.BINARIO.getMappings())
                .stream()
    ...
    public void inativar(Integer id) {
        Binario entity = repository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Código não encontrado: " + id));
