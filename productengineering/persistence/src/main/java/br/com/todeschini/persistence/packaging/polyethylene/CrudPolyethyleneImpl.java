...
@Component
public class CrudPolyethyleneImpl implements CrudPolyethylene {

    private final PolyethyleneRepository repository;
    private final PolyethyleneDomainToEntityAdapter adapter;
    private final EntityService entityService;

    public CrudPolyethyleneImpl(PolyethyleneRepository repository, PolyethyleneDomainToEntityAdapter adapter, EntityService entityService) {
    ...
    public Collection<? extends DPolyethylene> findByDescription(String description) {
        return repository.findByDescription(description).stream().map(adapter::toDomain).toList();
    }

    @Override
    @Transactional(readOnly = true)
    public List<DPolyethylene> findAllActiveAndCurrentOne(Long obj) {
    ...
    @Override
    @Transactional
    public DPolyethylene insert(DPolyethylene obj) {
    ...
    @Override
    @Transactional
    public DPolyethylene update(Long id, DPolyethylene obj) {
        if(!repository.existsById(id)){
            throw new ResourceNotFoundException("Código não encontrado: " + id);
        }
        Polyethylene entity = adapter.toEntity(obj);
    ...
    public void inactivate(Long id) {
        Polyethylene entity = repository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Código não encontrado: " + id));
    ...
    private void setCreationProperties(Polyethylene obj){
