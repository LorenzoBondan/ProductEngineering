...
@Component
public class CrudItemImpl implements CrudItem {

    private final ItemRepository repository;
    private final ItemDomainToEntityAdapter adapter;
    private final EntityService entityService;

    public CrudItemImpl(ItemRepository repository, ItemDomainToEntityAdapter adapter, EntityService entityService) {
    ...
    public Collection<? extends DItem> findByDescription(String description) {
        return repository.findByDescription(description).stream().map(adapter::toDomain).toList();
    }

    @Override
    @Transactional(readOnly = true)
    public List<DItem> findAllActiveAndCurrentOne(Long obj) {
    ...
    @Override
    @Transactional
    public DItem update(Long id, DItem obj) {
        if(!repository.existsById(id)){
            throw new ResourceNotFoundException("Código não encontrado: " + id);
        }
        Item entity = adapter.toEntity(obj);
    ...
    public void inactivate(Long id) {
        Item entity = repository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Código não encontrado: " + id));
    ...
    private void setCreationProperties(Item obj){
