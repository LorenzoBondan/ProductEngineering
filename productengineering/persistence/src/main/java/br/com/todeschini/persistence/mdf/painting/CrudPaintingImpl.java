...
public class CrudPaintingImpl implements CrudPainting {
    ...
    public Collection<? extends DPainting> findByColorAndPaintingType(Long colorId, Long paintingTypeId) {
        return repository.findByColorAndPaintingType(colorId, paintingTypeId).stream().map(adapter::toDomain).toList();
    }
