...
public class AcessorioServiceImplTest {

    private CrudAcessorio crud;
    private ConversaoValores conversaoValores;
    private AcessorioServiceImpl service;

    @BeforeEach
    void setup() {
        crud = mock(CrudAcessorio.class);
        conversaoValores = mock(ConversaoValores.class);
        service = new AcessorioServiceImpl(crud, conversaoValores);
    }

    @Test
    void deveBuscarTodos() {
        PageableRequest request = new PageableRequest(1,1,null,null,null,null,null);
        Paged<DAcessorio> pagedResult = new Paged<>();
        when(crud.buscarTodos(request)).thenReturn(pagedResult);

        Paged<DAcessorio> result = service.buscar(request);
    ...
    void deveBuscarPorId() {
        DAcessorio domain = AcessorioFactory.createDAcessorio();
        when(crud.buscar(1)).thenReturn(domain);

        DAcessorio result = service.buscar(domain.getCodigo());
    ...
    void deveIncluir() {
        ...
        DAcessorio result = service.incluir(domain);
    ...
    void deveLancarExcecaoAoIncluirDuplicado() {
        DAcessorio domain = AcessorioFactory.createDuplicatedDAcessorio("Duplicado");
        when(crud.pesquisarPorDescricao("Duplicado"))
                .thenReturn(List.of(new DAcessorio(1, "Duplicado", null,null,null,null,null)));
    ...
    void deveAtualizar() {
        ...
        DAcessorio result = service.atualizar(domain);
    ...
    void deveAtualizarEmLoteComSucesso() throws NoSuchFieldException {
        ...
        DAcessorio domain1 = AcessorioFactory.createDAcessorio();
        DAcessorio domain2 = AcessorioFactory.createDAcessorio();
        domain2.setCodigo(2);

        Field updatedField = DAcessorio.class.getDeclaredField("valor");

        when(crud.buscar(anyInt())).thenReturn(domain1, domain2);
        when(crud.atualizarEmLote(anyList())).thenReturn(List.of(domain1, domain2));
        when(conversaoValores.buscarCampoNaHierarquia(eq(DAcessorio.class), eq("valor"))).thenReturn(updatedField);
        when(conversaoValores.convertValor(eq(Double.class), any())).thenAnswer(invocation -> invocation.getArgument(1));

        List<DAcessorio> atualizados = service.atualizarEmLote(codigos, atributos, valores);
    ...
    void deveSubstituirPorVersaoAntigaComSucesso() {
        ...
        DAcessorio versaoAntiga = new DAcessorio(versionId, "Versão Antiga", null, null, null, null, null);
        when(crud.substituirPorVersaoAntiga(id, versionId)).thenReturn(versaoAntiga);

        DAcessorio substituido = service.substituirPorVersaoAntiga(id, versionId);
    ...
    void deveBuscarHistorico() {
        List<DHistory<DAcessorio>> historico = List.of();
        when(crud.buscarHistorico(1)).thenReturn(historico);

        List<DHistory<DAcessorio>> result = service.buscarHistorico(1);
