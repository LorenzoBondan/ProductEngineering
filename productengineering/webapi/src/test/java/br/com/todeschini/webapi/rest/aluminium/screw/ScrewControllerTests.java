...
import br.com.todeschini.webapi.api.v1.rest.aluminium.screw.ScrewController;
...
@Transactional
public class ScrewControllerTests {

    private final ScrewService service = mock(ScrewService.class);
    private final ScrewRepository repository = mock(ScrewRepository.class);
    private final ScrewController controller = new ScrewController(service, repository);

    private DScrew object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DScrew> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DScrew(existingId);
        nonExistingObject = new DScrew(nonExistingId);
        ...
        // insert
        when(service.insert(any(DScrew.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DScrew.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof ScrewDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndDescriptionContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(ScrewDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DScrew.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
