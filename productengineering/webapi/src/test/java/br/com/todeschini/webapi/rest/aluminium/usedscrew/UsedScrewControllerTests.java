...
import br.com.todeschini.webapi.api.v1.rest.aluminium.usedscrew.UsedScrewController;
...
@Transactional
public class UsedScrewControllerTests {

    private final UsedScrewService service = mock(UsedScrewService.class);
    private final UsedScrewRepository repository = mock(UsedScrewRepository.class);
    private final UsedScrewController controller = new UsedScrewController(service, repository);

    private DUsedScrew object, nonExistingObject;
    private Long existingId, nonExistingId;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DUsedScrew> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DUsedScrew(existingId);
        nonExistingObject = new DUsedScrew(nonExistingId);
        ...
        // insert
        when(service.insert(any(DUsedScrew.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DUsedScrew.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof UsedScrewDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusIn(eq(statusList),
                eq(pageable), eq(UsedScrewDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DUsedScrew.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
