...
import br.com.todeschini.webapi.api.v1.rest.aluminium.aluminiumson.AluminiumSonController;
...
@Transactional
public class AluminiumSonControllerTests {

    private final AluminiumSonService service = mock(AluminiumSonService.class);
    private final AluminiumSonRepository repository = mock(AluminiumSonRepository.class);
    private final AluminiumSonController controller = new AluminiumSonController(service, repository);

    private DAluminiumSon object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DAluminiumSon> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DAluminiumSon(existingId);
        nonExistingObject = new DAluminiumSon(nonExistingId);
        ...
        // insert
        when(service.insert(any(DAluminiumSon.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DAluminiumSon.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof AluminiumSonDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndDescriptionContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(AluminiumSonDTO.class));
    }
