...
import br.com.todeschini.webapi.api.v1.rest.mdf.paintingtype.PaintingTypeController;
...
@Transactional
public class PaintingTypeControllerTests {

    private final PaintingTypeService service = mock(PaintingTypeService.class);
    private final PaintingTypeRepository repository = mock(PaintingTypeRepository.class);
    private final PaintingTypeController controller = new PaintingTypeController(service, repository);

    private DPaintingType object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DPaintingType> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DPaintingType(existingId);
        nonExistingObject = new DPaintingType(nonExistingId);
        ...
        // insert
        when(service.insert(any(DPaintingType.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DPaintingType.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof PaintingTypeDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndDescriptionContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(PaintingTypeDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DPaintingType.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
