...
public class MaterialControllerTests {
    ...
    private final MaterialService service = mock(MaterialService.class);
    private final MaterialRepository repository = mock(MaterialRepository.class);
    private final MaterialController controller = new MaterialController(service, repository);

    private DMaterial object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DMaterial> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DMaterial(existingId);
        nonExistingObject = new DMaterial(nonExistingId);
        ...
        // insert
        when(service.insert(any(DMaterial.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DMaterial.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof MaterialDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndNameContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(MaterialDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DMaterial.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
