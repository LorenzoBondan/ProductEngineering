...
import br.com.todeschini.webapi.api.v1.rest.packaging.nonwovenfabric.NonwovenFabricController;
...
@Transactional
public class NonwovenFabricControllerTests {

    private final NonwovenFabricService service = mock(NonwovenFabricService.class);
    private final NonwovenFabricRepository repository = mock(NonwovenFabricRepository.class);
    private final NonwovenFabricController controller = new NonwovenFabricController(service, repository);

    private DNonwovenFabric object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DNonwovenFabric> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DNonwovenFabric(existingId);
        nonExistingObject = new DNonwovenFabric(nonExistingId);
        ...
        // insert
        when(service.insert(any(DNonwovenFabric.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DNonwovenFabric.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof NonwovenFabricDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndDescriptionContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(NonwovenFabricDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DNonwovenFabric.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
