...
import br.com.todeschini.webapi.api.v1.rest.mdf.usedpainting.UsedPaintingController;
...
@Transactional
public class UsedPaintingControllerTests {

    private final UsedPaintingService service = mock(UsedPaintingService.class);
    private final UsedPaintingRepository repository = mock(UsedPaintingRepository.class);
    private final UsedPaintingController controller = new UsedPaintingController(service, repository);

    private DUsedPainting object, nonExistingObject;
    private Long existingId, nonExistingId;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DUsedPainting> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DUsedPainting(existingId);
        nonExistingObject = new DUsedPainting(nonExistingId);
        ...
        // insert
        when(service.insert(any(DUsedPainting.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DUsedPainting.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof UsedPaintingDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusIn(eq(statusList),
                eq(pageable), eq(UsedPaintingDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DUsedPainting.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
