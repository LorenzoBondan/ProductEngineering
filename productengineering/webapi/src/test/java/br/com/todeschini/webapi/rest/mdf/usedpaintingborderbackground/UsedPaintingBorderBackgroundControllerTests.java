...
import br.com.todeschini.webapi.api.v1.rest.mdf.usedpaintingborderbackground.UsedPaintingBorderBackgroundController;
...
@Transactional
public class UsedPaintingBorderBackgroundControllerTests {

    private final UsedPaintingBorderBackgroundService service = mock(UsedPaintingBorderBackgroundService.class);
    private final UsedPaintingBorderBackgroundRepository repository = mock(UsedPaintingBorderBackgroundRepository.class);
    private final UsedPaintingBorderBackgroundController controller = new UsedPaintingBorderBackgroundController(service, repository);

    private DUsedPaintingBorderBackground object, nonExistingObject;
    private Long existingId, nonExistingId;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DUsedPaintingBorderBackground> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DUsedPaintingBorderBackground(existingId);
        nonExistingObject = new DUsedPaintingBorderBackground(nonExistingId);
        ...
        // insert
        when(service.insert(any(DUsedPaintingBorderBackground.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DUsedPaintingBorderBackground.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof UsedPaintingBorderBackgroundDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusIn(eq(statusList),
                eq(pageable), eq(UsedPaintingBorderBackgroundDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DUsedPaintingBorderBackground.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
