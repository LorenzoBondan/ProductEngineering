...
import br.com.todeschini.webapi.api.v1.rest.mdp.usedsheet.UsedSheetController;
...
@Transactional
public class UsedSheetControllerTests {

    private final UsedSheetService service = mock(UsedSheetService.class);
    private final UsedSheetRepository repository = mock(UsedSheetRepository.class);
    private final UsedSheetController controller = new UsedSheetController(service, repository);

    private DUsedSheet object, nonExistingObject;
    private Long existingId, nonExistingId;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DUsedSheet> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DUsedSheet(existingId);
        nonExistingObject = new DUsedSheet(nonExistingId);
        ...
        // insert
        when(service.insert(any(DUsedSheet.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DUsedSheet.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof UsedSheetDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusIn(eq(statusList),
                eq(pageable), eq(UsedSheetDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DUsedSheet.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
