...
import br.com.todeschini.webapi.api.v1.rest.mdf.polyester.PolyesterController;
...
@Transactional
public class PolyesterControllerTests {

    private final PolyesterService service = mock(PolyesterService.class);
    private final PolyesterRepository repository = mock(PolyesterRepository.class);
    private final PolyesterController controller = new PolyesterController(service, repository);

    private DPolyester object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DPolyester> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DPolyester(existingId);
        nonExistingObject = new DPolyester(nonExistingId);
        ...
        // insert
        when(service.insert(any(DPolyester.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DPolyester.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof PolyesterDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndDescriptionContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(PolyesterDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DPolyester.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
