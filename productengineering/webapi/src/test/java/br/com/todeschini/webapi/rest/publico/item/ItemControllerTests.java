...
import br.com.todeschini.webapi.api.v1.rest.publico.item.ItemController;
...
public class ItemControllerTests {
    ...
    private final ItemController controller = new ItemController(service, repository);

    private DItem object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DItem> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DItem(existingId);
        nonExistingObject = new DItem(nonExistingId);
        ...
        // insert
        when(service.insert(any(DItem.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DItem.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof ItemDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndDescriptionContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(ItemDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DItem.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
