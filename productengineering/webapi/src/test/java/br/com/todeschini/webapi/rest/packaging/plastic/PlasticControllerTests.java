...
import br.com.todeschini.webapi.api.v1.rest.packaging.plastic.PlasticController;
...
@Transactional
public class PlasticControllerTests {

    private final PlasticService service = mock(PlasticService.class);
    private final PlasticRepository repository = mock(PlasticRepository.class);
    private final PlasticController controller = new PlasticController(service, repository);

    private DPlastic object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DPlastic> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DPlastic(existingId);
        nonExistingObject = new DPlastic(nonExistingId);
        ...
        // insert
        when(service.insert(any(DPlastic.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DPlastic.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof PlasticDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndDescriptionContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(PlasticDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DPlastic.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
