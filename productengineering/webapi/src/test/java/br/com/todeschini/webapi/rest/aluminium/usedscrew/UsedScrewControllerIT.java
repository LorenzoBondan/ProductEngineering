...
import br.com.todeschini.persistence.aluminium.usedscrew.CrudUsedScrewImpl;
...
public class UsedScrewControllerIT extends BaseControllerIT<DUsedScrew> {

    @MockBean
    private CrudUsedScrewImpl crud;
    @MockBean
    private UsedScrewRepository repository;

    @BeforeEach
    void setUp() throws Exception {
        setConfiguration("/usedScrews");

        Long duplicatedId = 2L;

        factoryObject = UsedScrewFactory.createDUsedScrew();
        duplicatedObject = UsedScrewFactory.createDuplicatedDUsedScrew(duplicatedId);
        nonExistingObject = UsedScrewFactory.createNonExistingDUsedScrew(nonExistingId);
        ...
        // custom duplicated
        doThrow(DuplicatedResourceException.class).when(crud).findByScrewAndAluminiumSon(duplicatedId, duplicatedId);
    }
    ...
    public void validationShouldThrowUnprocessableEntityWhenInvalidDataTest() throws Exception {
        ...
        factoryObject = UsedScrewFactory.createDUsedScrew();
        factoryObject.setScrewCode(null);
        validate(factoryObject);

        factoryObject = UsedScrewFactory.createDUsedScrew();
        factoryObject.setScrewCode(-1L);
        validate(factoryObject);
