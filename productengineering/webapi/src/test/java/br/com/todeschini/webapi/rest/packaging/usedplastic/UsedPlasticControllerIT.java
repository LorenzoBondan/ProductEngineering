...
import br.com.todeschini.persistence.packaging.usedplastic.CrudUsedPlasticImpl;
...
public class UsedPlasticControllerIT extends BaseControllerIT<DUsedPlastic> {

    @MockBean
    private CrudUsedPlasticImpl crud;
    @MockBean
    private UsedPlasticRepository repository;

    @BeforeEach
    void setUp() throws Exception {
        setConfiguration("/usedPlastics");

        Long duplicatedId = 2L;
        String duplicatedGhostCode = "221F99";

        factoryObject = UsedPlasticFactory.createDUsedPlastic();
        duplicatedObject = UsedPlasticFactory.createDuplicatedDUsedPlastic(duplicatedId, duplicatedGhostCode);
        nonExistingObject = UsedPlasticFactory.createNonExistingDUsedPlastic(nonExistingId);
        ...
        // custom duplicated
        doThrow(DuplicatedResourceException.class).when(crud).findByPlasticAndGhost(duplicatedId, duplicatedGhostCode);
    }
    ...
    public void validationShouldThrowUnprocessableEntityWhenInvalidDataTest() throws Exception {
        ...
        factoryObject = UsedPlasticFactory.createDUsedPlastic();
        factoryObject.setPlasticCode(null);
        validate(factoryObject);

        factoryObject = UsedPlasticFactory.createDUsedPlastic();
        factoryObject.setPlasticCode(ValidationConstants.longNegative);
        validate(factoryObject);
