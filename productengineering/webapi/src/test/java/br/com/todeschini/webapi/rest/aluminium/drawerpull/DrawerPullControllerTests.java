...
import br.com.todeschini.webapi.api.v1.rest.aluminium.drawerpull.DrawerPullController;
...
@Transactional
public class DrawerPullControllerTests {

    private final DrawerPullService service = mock(DrawerPullService.class);
    private final DrawerPullRepository repository = mock(DrawerPullRepository.class);
    private final DrawerPullController controller = new DrawerPullController(service, repository);

    private DDrawerPull object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DDrawerPull> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DDrawerPull(existingId);
        nonExistingObject = new DDrawerPull(nonExistingId);
        ...
        // insert
        when(service.insert(any(DDrawerPull.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DDrawerPull.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof DrawerPullDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndDescriptionContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(DrawerPullDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DDrawerPull.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
