...
import br.com.todeschini.webapi.api.v1.rest.mdf.back.BackController;
...
public class BackControllerTests {
    ...
    private final BackService service = mock(BackService.class);
    private final BackRepository repository = mock(BackRepository.class);
    private final BackController controller = new BackController(service, repository);

    private DBack object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DBack> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DBack(existingId);
        nonExistingObject = new DBack(nonExistingId);
        ...
        // insert
        when(service.insert(any(DBack.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DBack.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof BackDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndDescriptionContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(BackDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DBack.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
