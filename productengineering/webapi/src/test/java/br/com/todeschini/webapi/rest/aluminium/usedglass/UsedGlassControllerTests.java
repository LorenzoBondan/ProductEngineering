...
import br.com.todeschini.webapi.api.v1.rest.aluminium.usedglass.UsedGlassController;
...
@Transactional
public class UsedGlassControllerTests {

    private final UsedGlassService service = mock(UsedGlassService.class);
    private final UsedGlassRepository repository = mock(UsedGlassRepository.class);
    private final UsedGlassController controller = new UsedGlassController(service, repository);

    private DUsedGlass object, nonExistingObject;
    private Long existingId, nonExistingId;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DUsedGlass> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DUsedGlass(existingId);
        nonExistingObject = new DUsedGlass(nonExistingId);
        ...
        // insert
        when(service.insert(any(DUsedGlass.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DUsedGlass.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof UsedGlassDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusIn(eq(statusList),
                eq(pageable), eq(UsedGlassDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DUsedGlass.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
