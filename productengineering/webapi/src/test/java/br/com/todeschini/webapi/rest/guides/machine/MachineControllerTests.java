...
import br.com.todeschini.webapi.api.v1.rest.guides.machine.MachineController;
...
public class MachineControllerTests {
    ...
    private final MachineService service = mock(MachineService.class);
    private final MachineRepository repository = mock(MachineRepository.class);
    private final MachineController controller = new MachineController(service, repository);

    private DMachine object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DMachine> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DMachine(existingId);
        nonExistingObject = new DMachine(nonExistingId);
        ...
        // insert
        when(service.insert(any(DMachine.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DMachine.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof MachineDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndNameContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(MachineDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DMachine.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
