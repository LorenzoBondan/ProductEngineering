...
import br.com.todeschini.webapi.api.v1.rest.mdp.glue.GlueController;
...
@Transactional
public class GlueControllerTests {

    private final GlueService service = mock(GlueService.class);
    private final GlueRepository repository = mock(GlueRepository.class);
    private final GlueController controller = new GlueController(service, repository);

    private DGlue object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DGlue> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DGlue(existingId);
        nonExistingObject = new DGlue(nonExistingId);
        ...
        // insert
        when(service.insert(any(DGlue.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DGlue.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof GlueDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndDescriptionContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(GlueDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DGlue.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
