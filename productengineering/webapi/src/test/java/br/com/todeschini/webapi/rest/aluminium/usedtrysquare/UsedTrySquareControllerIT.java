...
import br.com.todeschini.persistence.aluminium.usedtrysquare.CrudUsedTrySquareImpl;
...
public class UsedTrySquareControllerIT extends BaseControllerIT<DUsedTrySquare> {

    @MockBean
    private CrudUsedTrySquareImpl crud;
    @MockBean
    private UsedTrySquareRepository repository;

    @BeforeEach
    void setUp() throws Exception {
        setConfiguration("/usedTrySquares");

        Long duplicatedId = 2L;

        factoryObject = UsedTrySquareFactory.createDUsedTrySquare();
        duplicatedObject = UsedTrySquareFactory.createDuplicatedDUsedTrySquare(duplicatedId);
        nonExistingObject = UsedTrySquareFactory.createNonExistingDUsedTrySquare(nonExistingId);
        ...
        // custom duplicated
        doThrow(DuplicatedResourceException.class).when(crud).findByTrySquareAndAluminiumSon(duplicatedId, duplicatedId);
    }
    ...
    public void validationShouldThrowUnprocessableEntityWhenInvalidDataTest() throws Exception {
        ...
        factoryObject = UsedTrySquareFactory.createDUsedTrySquare();
        factoryObject.setTrySquareCode(null);
        validate(factoryObject);

        factoryObject = UsedTrySquareFactory.createDUsedTrySquare();
        factoryObject.setTrySquareCode(-1L);
        validate(factoryObject);
