...
import br.com.todeschini.webapi.api.v1.rest.packaging.usedpolyethylene.UsedPolyethyleneController;
...
@Transactional
public class UsedPolyethylenesControllerTests {

    private final UsedPolyethyleneService service = mock(UsedPolyethyleneService.class);
    private final UsedPolyethyleneRepository repository = mock(UsedPolyethyleneRepository.class);
    private final UsedPolyethyleneController controller = new UsedPolyethyleneController(service, repository);

    private DUsedPolyethylene object, nonExistingObject;
    private Long existingId, nonExistingId;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DUsedPolyethylene> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DUsedPolyethylene(existingId);
        nonExistingObject = new DUsedPolyethylene(nonExistingId);
        ...
        // insert
        when(service.insert(any(DUsedPolyethylene.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DUsedPolyethylene.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof UsedPolyethyleneDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusIn(eq(statusList),
                eq(pageable), eq(UsedPolyethyleneDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DUsedPolyethylene.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
