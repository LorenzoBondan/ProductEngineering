...
import br.com.todeschini.webapi.api.v1.rest.publico.color.ColorController;
...
public class ColorControllerTests {
    ...
    private final ColorService service = mock(ColorService.class);
    private final ColorRepository repository = mock(ColorRepository.class);
    private final ColorController controller = new ColorController(service, repository);

    private DColor object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DColor> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DColor(existingId);
        nonExistingObject = new DColor(nonExistingId);
        ...
        // insert
        when(service.insert(any(DColor.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DColor.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof ColorDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndNameContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(ColorDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DColor.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
