...
import br.com.todeschini.webapi.api.v1.rest.aluminium.trysquare.TrySquareController;
...
@Transactional
public class TrySquareControllerTests {

    private final TrySquareService service = mock(TrySquareService.class);
    private final TrySquareRepository repository = mock(TrySquareRepository.class);
    private final TrySquareController controller = new TrySquareController(service, repository);

    private DTrySquare object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DTrySquare> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DTrySquare(existingId);
        nonExistingObject = new DTrySquare(nonExistingId);
        ...
        // insert
        when(service.insert(any(DTrySquare.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DTrySquare.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof TrySquareDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndDescriptionContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(TrySquareDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DTrySquare.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
