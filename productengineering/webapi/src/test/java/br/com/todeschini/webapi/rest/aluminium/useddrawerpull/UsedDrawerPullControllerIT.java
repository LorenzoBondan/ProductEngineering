...
import br.com.todeschini.persistence.aluminium.useddrawerpull.CrudUsedDrawerPullImpl;
...
public class UsedDrawerPullControllerIT extends BaseControllerIT<DUsedDrawerPull> {

    @MockBean
    private CrudUsedDrawerPullImpl crud;
    @MockBean
    private UsedDrawerPullRepository repository;

    @BeforeEach
    void setUp() throws Exception {
        setConfiguration("/usedDrawerPulls");

        Long duplicatedId = 2L;

        factoryObject = UsedDrawerPullFactory.createDUsedDrawerPull();
        duplicatedObject = UsedDrawerPullFactory.createDuplicatedDUsedDrawerPull(duplicatedId);
        nonExistingObject = UsedDrawerPullFactory.createNonExistingDUsedDrawerPull(nonExistingId);
        ...
        // custom duplicated
        doThrow(DuplicatedResourceException.class).when(crud).findByDrawerPullAndAluminiumSon(duplicatedId, duplicatedId);
    }
    ...
    public void validationShouldThrowUnprocessableEntityWhenInvalidDataTest() throws Exception {
        ...
        factoryObject = UsedDrawerPullFactory.createDUsedDrawerPull();
        factoryObject.setDrawerPullCode(null);
        validate(factoryObject);

        factoryObject = UsedDrawerPullFactory.createDUsedDrawerPull();
        factoryObject.setDrawerPullCode(-1L);
        validate(factoryObject);
        ...
        factoryObject = UsedDrawerPullFactory.createDUsedDrawerPull();
        factoryObject.setQuantity(negative);
        validate(factoryObject);
    }
}
