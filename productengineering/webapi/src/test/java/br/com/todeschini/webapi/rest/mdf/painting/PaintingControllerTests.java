...
import br.com.todeschini.webapi.api.v1.rest.mdf.painting.PaintingController;
...
public class PaintingControllerTests {
    ...
    private final PaintingService service = mock(PaintingService.class);
    private final PaintingRepository repository = mock(PaintingRepository.class);
    private final PaintingController controller = new PaintingController(service, repository);

    private DPainting object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DPainting> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DPainting(existingId);
        nonExistingObject = new DPainting(nonExistingId);
        ...
        // insert
        when(service.insert(any(DPainting.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DPainting.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof PaintingDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndDescriptionContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(PaintingDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DPainting.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
