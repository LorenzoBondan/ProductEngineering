...
import br.com.todeschini.webapi.api.v1.rest.mdf.paintingson.PaintingSonController;
...
@Transactional
public class PaintingSonControllerTests {

    private final PaintingSonService service = mock(PaintingSonService.class);
    private final PaintingSonRepository repository = mock(PaintingSonRepository.class);
    private final PaintingSonController controller = new PaintingSonController(service, repository);

    private DPaintingSon object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DPaintingSon> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DPaintingSon(existingId);
        nonExistingObject = new DPaintingSon(nonExistingId);
        ...
        // insert
        when(service.insert(any(DPaintingSon.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DPaintingSon.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof PaintingSonDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndDescriptionContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(PaintingSonDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DPaintingSon.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
