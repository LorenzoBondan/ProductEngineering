...
import br.com.todeschini.webapi.api.v1.rest.packaging.cornerbracket.CornerBracketController;
...
@Transactional
public class CornerBracketControllerTests {

    private final CornerBracketService service = mock(CornerBracketService.class);
    private final CornerBracketRepository repository = mock(CornerBracketRepository.class);
    private final CornerBracketController controller = new CornerBracketController(service, repository);

    private DCornerBracket object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DCornerBracket> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DCornerBracket(existingId);
        nonExistingObject = new DCornerBracket(nonExistingId);
        ...
        // insert
        when(service.insert(any(DCornerBracket.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DCornerBracket.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof CornerBracketDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndDescriptionContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(CornerBracketDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DCornerBracket.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
