...
import br.com.todeschini.webapi.api.v1.rest.aluminium.glass.GlassController;
...
@Transactional
public class GlassControllerTests {

    private final GlassService service = mock(GlassService.class);
    private final GlassRepository repository = mock(GlassRepository.class);
    private final GlassController controller = new GlassController(service, repository);

    private DGlass object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DGlass> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DGlass(existingId);
        nonExistingObject = new DGlass(nonExistingId);
        ...
        // insert
        when(service.insert(any(DGlass.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DGlass.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof GlassDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndDescriptionContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(GlassDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DGlass.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
