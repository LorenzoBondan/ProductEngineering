...
import br.com.todeschini.webapi.api.v1.rest.aluminium.useddrawerpull.UsedDrawerPullController;
...
@Transactional
public class UsedDrawerPullControllerTests {

    private final UsedDrawerPullService service = mock(UsedDrawerPullService.class);
    private final UsedDrawerPullRepository repository = mock(UsedDrawerPullRepository.class);
    private final UsedDrawerPullController controller = new UsedDrawerPullController(service, repository);

    private DUsedDrawerPull object, nonExistingObject;
    private Long existingId, nonExistingId;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DUsedDrawerPull> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DUsedDrawerPull(existingId);
        nonExistingObject = new DUsedDrawerPull(nonExistingId);
        ...
        // insert
        when(service.insert(any(DUsedDrawerPull.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DUsedDrawerPull.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof UsedDrawerPullDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusIn(eq(statusList),
                eq(pageable), eq(UsedDrawerPullDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DUsedDrawerPull.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
