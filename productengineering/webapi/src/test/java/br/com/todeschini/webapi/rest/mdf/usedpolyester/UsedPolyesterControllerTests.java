...
import br.com.todeschini.webapi.api.v1.rest.mdf.usedpolyester.UsedPolyesterController;
...
@Transactional
public class UsedPolyesterControllerTests {

    private final UsedPolyesterService service = mock(UsedPolyesterService.class);
    private final UsedPolyesterRepository repository = mock(UsedPolyesterRepository.class);
    private final UsedPolyesterController controller = new UsedPolyesterController(service, repository);

    private DUsedPolyester object, nonExistingObject;
    private Long existingId, nonExistingId;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DUsedPolyester> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DUsedPolyester(existingId);
        nonExistingObject = new DUsedPolyester(nonExistingId);
        ...
        // insert
        when(service.insert(any(DUsedPolyester.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DUsedPolyester.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof UsedPolyesterDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusIn(eq(statusList),
                eq(pageable), eq(UsedPolyesterDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DUsedPolyester.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
