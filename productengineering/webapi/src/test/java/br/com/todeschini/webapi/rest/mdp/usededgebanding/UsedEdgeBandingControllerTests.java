...
import br.com.todeschini.webapi.api.v1.rest.mdp.usededgebanding.UsedEdgeBandingController;
...
@Transactional
public class UsedEdgeBandingControllerTests {

    private final UsedEdgeBandingService service = mock(UsedEdgeBandingService.class);
    private final UsedEdgeBandingRepository repository = mock(UsedEdgeBandingRepository.class);
    private final UsedEdgeBandingController controller = new UsedEdgeBandingController(service, repository);

    private DUsedEdgeBanding object, nonExistingObject;
    private Long existingId, nonExistingId;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DUsedEdgeBanding> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DUsedEdgeBanding(existingId);
        nonExistingObject = new DUsedEdgeBanding(nonExistingId);
        ...
        // insert
        when(service.insert(any(DUsedEdgeBanding.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DUsedEdgeBanding.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof UsedEdgeBandingDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusIn(eq(statusList),
                eq(pageable), eq(UsedEdgeBandingDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DUsedEdgeBanding.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
