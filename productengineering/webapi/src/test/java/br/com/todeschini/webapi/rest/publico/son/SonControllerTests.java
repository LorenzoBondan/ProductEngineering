...
import br.com.todeschini.webapi.api.v1.rest.publico.son.SonController;
...
@Transactional
public class SonControllerTests {

    private final SonService service = mock(SonService.class);
    private final SonRepository repository = mock(SonRepository.class);
    private final SonController controller = new SonController(service, repository);

    private DSon object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DSon> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DSon(existingId);
        nonExistingObject = new DSon(nonExistingId);
        ...
        // insert
        when(service.insert(any(DSon.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DSon.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof SonDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndDescriptionContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(SonDTO.class));
    }
