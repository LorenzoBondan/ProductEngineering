...
public class CorControllerTest extends BaseControllerTest<DCor, Cor> {
    ...
    @Autowired
    public CorControllerTest(MockMvc mockMvc, ObjectMapper objectMapper, HistoryRepository historyRepository, TokenUtil tokenUtil) {
        super(mockMvc, objectMapper, historyRepository, tokenUtil);
        this.baseUrl = "/api/cor";
    }

    @BeforeAll
    public void setUp() {
        novaEntidade = CorFactory.createDCor();
        novaEntidade.setCodigo(null);
        super.setHasDescricao(true);
    }

    @Override
    protected Class<Cor> getEntityClass() {
        return Cor.class;
    }

    @Override
    protected String getDescricao(DCor entidade) {
        return entidade.getDescricao();
    }

    @Override
    protected void setCodigo(DCor entidade, Integer codigo) {
        entidade.setCodigo(codigo);
    }

    @Override
    protected void setDescricao(DCor entidade, String descricao) {
    ...
    void deveSubstituirVersaoTest() throws Exception {
        super.deveSubstituirVersao("tb_cor", "cdcor");
    }
    ...
    void tearDown() {
        if (idCriado != null) {
            corRepository.deleteById(idCriado);
            Map<String, Object> id = new HashMap<>();
            id.put("cdcor", idCriado);
