...
import br.com.todeschini.webapi.api.v1.rest.publico.attachment.AttachmentController;
...
public class AttachmentControllerTests {
    ...
    private final AttachmentController controller = new AttachmentController(service, repository);

    private DAttachment object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DAttachment> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DAttachment(existingId);
        nonExistingObject = new DAttachment(nonExistingId);
        ...
        // insert
        when(service.insert(any(DAttachment.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DAttachment.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof AttachmentDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndDescriptionContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(AttachmentDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DAttachment.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
