...
import br.com.todeschini.webapi.api.v1.rest.packaging.usednonwovenfabric.UsedNonwovenFabricController;
...
@Transactional
public class UsedNonwovenFabricControllerTests {

    private final UsedNonwovenFabricService service = mock(UsedNonwovenFabricService.class);
    private final UsedNonwovenFabricRepository repository = mock(UsedNonwovenFabricRepository.class);
    private final UsedNonwovenFabricController controller = new UsedNonwovenFabricController(service, repository);

    private DUsedNonwovenFabric object, nonExistingObject;
    private Long existingId, nonExistingId;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DUsedNonwovenFabric> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DUsedNonwovenFabric(existingId);
        nonExistingObject = new DUsedNonwovenFabric(nonExistingId);
        ...
        // insert
        when(service.insert(any(DUsedNonwovenFabric.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DUsedNonwovenFabric.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof UsedNonwovenFabricDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusIn(eq(statusList),
                eq(pageable), eq(UsedNonwovenFabricDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DUsedNonwovenFabric.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
