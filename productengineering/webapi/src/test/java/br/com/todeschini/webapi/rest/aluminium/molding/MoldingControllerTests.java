...
import br.com.todeschini.webapi.api.v1.rest.aluminium.molding.MoldingController;
...
@Transactional
public class MoldingControllerTests {

    private final MoldingService service = mock(MoldingService.class);
    private final MoldingRepository repository = mock(MoldingRepository.class);
    private final MoldingController controller = new MoldingController(service, repository);

    private DMolding object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DMolding> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DMolding(existingId);
        nonExistingObject = new DMolding(nonExistingId);
        ...
        // insert
        when(service.insert(any(DMolding.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DMolding.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof MoldingDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndDescriptionContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(MoldingDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DMolding.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
