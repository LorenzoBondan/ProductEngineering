...
import br.com.todeschini.webapi.api.v1.rest.aluminium.usedmolding.UsedMoldingController;
...
@Transactional
public class UsedMoldingControllerTests {

    private final UsedMoldingService service = mock(UsedMoldingService.class);
    private final UsedMoldingRepository repository = mock(UsedMoldingRepository.class);
    private final UsedMoldingController controller = new UsedMoldingController(service, repository);

    private DUsedMolding object, nonExistingObject;
    private Long existingId, nonExistingId;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DUsedMolding> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DUsedMolding(existingId);
        nonExistingObject = new DUsedMolding(nonExistingId);
        ...
        // insert
        when(service.insert(any(DUsedMolding.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DUsedMolding.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof UsedMoldingDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusIn(eq(statusList),
                eq(pageable), eq(UsedMoldingDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DUsedMolding.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
