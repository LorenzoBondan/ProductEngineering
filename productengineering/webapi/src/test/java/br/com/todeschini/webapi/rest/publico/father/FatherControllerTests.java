...
import br.com.todeschini.webapi.api.v1.rest.publico.father.FatherController;
...
@Transactional
public class FatherControllerTests {

    private final FatherService service = mock(FatherService.class);
    private final FatherRepository repository = mock(FatherRepository.class);
    private final FatherController controller = new FatherController(service, repository);

    private DFather object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DFather> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DFather(existingId);
        nonExistingObject = new DFather(nonExistingId);
        ...
        // insert
        when(service.insert(any(DFather.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DFather.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof FatherDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndDescriptionContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(FatherDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DFather.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
