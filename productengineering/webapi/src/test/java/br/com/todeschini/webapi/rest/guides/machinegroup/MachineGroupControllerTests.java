...
import br.com.todeschini.webapi.api.v1.rest.guides.machinegroup.MachineGroupController;
...
@Transactional
public class MachineGroupControllerTests {

    private final MachineGroupService service = mock(MachineGroupService.class);
    private final MachineGroupRepository repository = mock(MachineGroupRepository.class);
    private final MachineGroupController controller = new MachineGroupController(service, repository);

    private DMachineGroup object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DMachineGroup> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DMachineGroup(existingId);
        nonExistingObject = new DMachineGroup(nonExistingId);
        ...
        // insert
        when(service.insert(any(DMachineGroup.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DMachineGroup.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof MachineGroupDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndNameContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(MachineGroupDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DMachineGroup.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
