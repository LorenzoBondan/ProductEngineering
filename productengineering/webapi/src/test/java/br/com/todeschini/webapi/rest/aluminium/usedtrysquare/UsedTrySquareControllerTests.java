...
import br.com.todeschini.webapi.api.v1.rest.aluminium.usedtrysquare.UsedTrySquareController;
...
@Transactional
public class UsedTrySquareControllerTests {

    private final UsedTrySquareService service = mock(UsedTrySquareService.class);
    private final UsedTrySquareRepository repository = mock(UsedTrySquareRepository.class);
    private final UsedTrySquareController controller = new UsedTrySquareController(service, repository);

    private DUsedTrySquare object, nonExistingObject;
    private Long existingId, nonExistingId;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DUsedTrySquare> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DUsedTrySquare(existingId);
        nonExistingObject = new DUsedTrySquare(nonExistingId);
        ...
        // insert
        when(service.insert(any(DUsedTrySquare.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DUsedTrySquare.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof UsedTrySquareDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusIn(eq(statusList),
                eq(pageable), eq(UsedTrySquareDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DUsedTrySquare.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
