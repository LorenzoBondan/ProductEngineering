...
import br.com.todeschini.webapi.api.v1.rest.packaging.polyethylene.PolyethyleneController;
...
@Transactional
public class PolyethyleneControllerTests {

    private final PolyethyleneService service = mock(PolyethyleneService.class);
    private final PolyethyleneRepository repository = mock(PolyethyleneRepository.class);
    private final PolyethyleneController controller = new PolyethyleneController(service, repository);

    private DPolyethylene object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DPolyethylene> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DPolyethylene(existingId);
        nonExistingObject = new DPolyethylene(nonExistingId);
        ...
        // insert
        when(service.insert(any(DPolyethylene.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DPolyethylene.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof PolyethyleneDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndDescriptionContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(PolyethyleneDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DPolyethylene.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
