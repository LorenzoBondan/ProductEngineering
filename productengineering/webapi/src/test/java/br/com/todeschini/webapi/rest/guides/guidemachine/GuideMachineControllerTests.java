...
import br.com.todeschini.webapi.api.v1.rest.guides.guidemachine.GuideMachineController;
...
@Transactional
public class GuideMachineControllerTests {

    private final GuideMachineService service = mock(GuideMachineService.class);
    private final GuideMachineRepository repository = mock(GuideMachineRepository.class);
    private final GuideMachineController controller = new GuideMachineController(service, repository);

    private DGuideMachine object, nonExistingObject;
    private Long existingId, nonExistingId;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DGuideMachine> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DGuideMachine(existingId);
        nonExistingObject = new DGuideMachine(nonExistingId);
        ...
        // insert
        when(service.insert(any(DGuideMachine.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DGuideMachine.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof GuideMachineDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusIn(eq(statusList),
                eq(pageable), eq(GuideMachineDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DGuideMachine.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
