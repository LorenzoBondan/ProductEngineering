...
import br.com.todeschini.webapi.api.v1.rest.packaging.usedcornerbracket.UsedCornerBracketController;
...
@Transactional
public class UsedCornerBracketControllerTests {

    private final UsedCornerBracketService service = mock(UsedCornerBracketService.class);
    private final UsedCornerBracketRepository repository = mock(UsedCornerBracketRepository.class);
    private final UsedCornerBracketController controller = new UsedCornerBracketController(service, repository);

    private DUsedCornerBracket object, nonExistingObject;
    private Long existingId, nonExistingId;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DUsedCornerBracket> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DUsedCornerBracket(existingId);
        nonExistingObject = new DUsedCornerBracket(nonExistingId);
        ...
        // insert
        when(service.insert(any(DUsedCornerBracket.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DUsedCornerBracket.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof UsedCornerBracketDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusIn(eq(statusList),
                eq(pageable), eq(UsedCornerBracketDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DUsedCornerBracket.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
