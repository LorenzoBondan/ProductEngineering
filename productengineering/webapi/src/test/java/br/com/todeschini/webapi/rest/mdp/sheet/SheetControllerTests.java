...
import br.com.todeschini.webapi.api.v1.rest.mdp.sheet.SheetController;
...
public class SheetControllerTests {
    ...
    private final SheetService service = mock(SheetService.class);
    private final SheetRepository repository = mock(SheetRepository.class);
    private final SheetController controller = new SheetController(service, repository);
    ...
    void setUp() {
        ...
        object = new DSheet(existingId);
        nonExistingObject = new DSheet(nonExistingId);
        ...
        // insert
        when(service.insert(any(DSheet.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DSheet.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof SheetDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndDescriptionContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(SheetDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DSheet.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
