...
import br.com.todeschini.webapi.api.v1.rest.mdp.usedglue.UsedGlueController;
...
@Transactional
public class UsedGlueControllerTests {

    private final UsedGlueService service = mock(UsedGlueService.class);
    private final UsedGlueRepository repository = mock(UsedGlueRepository.class);
    private final UsedGlueController controller = new UsedGlueController(service, repository);

    private DUsedGlue object, nonExistingObject;
    private Long existingId, nonExistingId;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DUsedGlue> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DUsedGlue(existingId);
        nonExistingObject = new DUsedGlue(nonExistingId);
        ...
        // insert
        when(service.insert(any(DUsedGlue.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DUsedGlue.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof UsedGlueDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusIn(eq(statusList),
                eq(pageable), eq(UsedGlueDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DUsedGlue.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
