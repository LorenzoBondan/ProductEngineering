...
import br.com.todeschini.webapi.api.v1.rest.mdp.edgebanding.EdgeBandingController;
...
@Transactional
public class EdgeBandingControllerTests {

    private final EdgeBandingService service = mock(EdgeBandingService.class);
    private final EdgeBandingRepository repository = mock(EdgeBandingRepository.class);
    private final EdgeBandingController controller = new EdgeBandingController(service, repository);

    private DEdgeBanding object, nonExistingObject;
    private Long existingId, nonExistingId;
    private String description;
    private final List<Status> statusList = new ArrayList<>();
    private final List<DEdgeBanding> objectList = new ArrayList<>();
    ...
    void setUp() {
        ...
        object = new DEdgeBanding(existingId);
        nonExistingObject = new DEdgeBanding(nonExistingId);
        ...
        // insert
        when(service.insert(any(DEdgeBanding.class))).thenReturn(object);

        // update
        when(service.update(eq(existingId), any(DEdgeBanding.class))).thenReturn(object);
    ...
    void findByStatusInAndNameContainingIgnoreCaseShouldReturnPage() {
        ...
        boolean allMatch = responseBody.stream().allMatch(obj -> obj instanceof EdgeBandingDTO);
        assertThat(allMatch).isTrue();

        verify(repository, times(1)).findByStatusInAndDescriptionContainingIgnoreCase(eq(statusList),
                eq(description), eq(pageable), eq(EdgeBandingDTO.class));
    }
    ...
    public void findByIdShouldReturnObjectWhenIdExists() {
        ...
        // Verificando o tipo de dado e código de status
        assertThat(responseEntity.getBody()).isInstanceOf(DEdgeBanding.class);
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
