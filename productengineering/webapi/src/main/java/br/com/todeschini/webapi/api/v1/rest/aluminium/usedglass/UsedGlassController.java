package br.com.todeschini.webapi.api.v1.rest.aluminium.usedglass;
...
import br.com.todeschini.webapi.api.v1.rest.aluminium.usedglass.projection.UsedGlassDTO;
...
@Tag(name = "UsedGlasses")
@RestController
@RequestMapping(value = "/usedGlasses")
public class UsedGlassController {

    private final UsedGlassService service;
    private final UsedGlassRepository repository;

    public UsedGlassController(UsedGlassService service, UsedGlassRepository repository) {
    ...
    public ResponseEntity<?> findByStatusIn(@RequestBody(required = false) List<Status> statusList,
        ...
        return ResponseEntity.ok().body(repository.findByStatusIn(statusList, pageable, UsedGlassDTO.class));
    }
    ...
    public ResponseEntity<List<DUsedGlass>> findAllActiveAndCurrentOne(@RequestParam(value = "id", required = false) Long id){
        List<DUsedGlass> list = service.findAllActiveAndCurrentOne(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @GetMapping(value = "/{id}")
    public ResponseEntity<DUsedGlass> findById(@PathVariable Long id){
        DUsedGlass dto = service.find(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @PostMapping
    public ResponseEntity<DUsedGlass> insert(@Valid @RequestBody DUsedGlass dto){
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @PutMapping(value = "/{id}")
    public ResponseEntity<DUsedGlass> update(@PathVariable Long id, @Valid @RequestBody DUsedGlass dto){
    ...
    public ResponseEntity<DUsedGlass> inactivate(@PathVariable Long id){
    ...
    public ResponseEntity<DUsedGlass> delete(@PathVariable Long id){
        service.delete(id);
