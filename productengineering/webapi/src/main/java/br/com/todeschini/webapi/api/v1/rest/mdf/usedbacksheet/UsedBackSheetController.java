package br.com.todeschini.webapi.api.v1.rest.mdf.usedbacksheet;
...
@Tag(name = "UsedBackSheets")
@RestController
@RequestMapping(value = "/usedBackSheets")
public class UsedBackSheetController {

    private final UsedBackSheetService service;
    private final UsedBackSheetRepository repository;

    public UsedBackSheetController(UsedBackSheetService service, UsedBackSheetRepository repository) {
    ...
    public ResponseEntity<?> findByStatusIn(@RequestParam(value = "status", required = false) String statusParam,
        ...
        return ResponseEntity.ok().body(repository.findByStatusIn(statusList, pageable, UsedBackSheetDTO.class));
    }
    ...
    public ResponseEntity<List<DUsedBackSheet>> findAllActiveAndCurrentOne(@RequestParam(value = "id", required = false) Long id){
        List<DUsedBackSheet> list = service.findAllActiveAndCurrentOne(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @GetMapping(value = "/{id}")
    public ResponseEntity<DUsedBackSheet> findById(@PathVariable Long id){
        DUsedBackSheet dto = service.find(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @PostMapping
    public ResponseEntity<DUsedBackSheet> insert(@Valid @RequestBody DUsedBackSheet dto){
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @PutMapping(value = "/{id}")
    public ResponseEntity<DUsedBackSheet> update(@PathVariable Long id, @Valid @RequestBody DUsedBackSheet dto){
    ...
    public ResponseEntity<DUsedBackSheet> inactivate(@PathVariable Long id){
    ...
    public ResponseEntity<DUsedBackSheet> delete(@PathVariable Long id){
        service.delete(id);
