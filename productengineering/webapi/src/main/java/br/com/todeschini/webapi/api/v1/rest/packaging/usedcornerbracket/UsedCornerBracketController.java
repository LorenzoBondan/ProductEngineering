package br.com.todeschini.webapi.api.v1.rest.packaging.usedcornerbracket;
...
import br.com.todeschini.webapi.api.v1.rest.packaging.usedcornerbracket.projection.UsedCornerBracketDTO;
...
@Tag(name = "UsedCornerBrackets")
@RestController
@RequestMapping(value = "/usedCornerBrackets")
public class UsedCornerBracketController {

    private final UsedCornerBracketService service;
    private final UsedCornerBracketRepository repository;

    public UsedCornerBracketController(UsedCornerBracketService service, UsedCornerBracketRepository repository) {
    ...
    public ResponseEntity<?> findByStatusIn(@RequestParam(value = "status", required = false) String statusParam,
        ...
        return ResponseEntity.ok().body(repository.findByStatusIn(statusList, pageable, UsedCornerBracketDTO.class));
    }
    ...
    public ResponseEntity<List<DUsedCornerBracket>> findAllActiveAndCurrentOne(@RequestParam(value = "id", required = false) Long id){
        List<DUsedCornerBracket> list = service.findAllActiveAndCurrentOne(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_ANALYST', 'ROLE_OPERATOR')")
    @GetMapping(value = "/{id}")
    public ResponseEntity<DUsedCornerBracket> findById(@PathVariable Long id){
        DUsedCornerBracket dto = service.find(id);
    ...
    @PostMapping
    public ResponseEntity<DUsedCornerBracket> insert(@Valid @RequestBody DUsedCornerBracket dto){
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_ANALYST')")
    @PutMapping(value = "/{id}")
    public ResponseEntity<DUsedCornerBracket> update(@PathVariable Long id, @Valid @RequestBody DUsedCornerBracket dto){
    ...
    public ResponseEntity<DUsedCornerBracket> inactivate(@PathVariable Long id){
    ...
    public ResponseEntity<DUsedCornerBracket> delete(@PathVariable Long id){
        service.delete(id);
