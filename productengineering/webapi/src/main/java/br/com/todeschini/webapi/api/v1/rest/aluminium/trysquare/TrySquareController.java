package br.com.todeschini.webapi.api.v1.rest.aluminium.trysquare;
...
@Tag(name = "TrySquares")
@RestController
@RequestMapping(value = "/trySquares")
public class TrySquareController {

    private final TrySquareService service;
    private final TrySquareRepository repository;

    public TrySquareController(TrySquareService service, TrySquareRepository repository) {
    ...
    public ResponseEntity<?> findByStatusInAndDescriptionContainingIgnoreCase(@RequestParam(value = "status", required = false) String statusParam,
        ...
        return ResponseEntity.ok().body(repository.findByStatusInAndDescriptionContainingIgnoreCase(statusList, description, pageable, TrySquareDTO.class));
    }
