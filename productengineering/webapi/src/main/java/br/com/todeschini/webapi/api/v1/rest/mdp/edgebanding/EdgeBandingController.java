package br.com.todeschini.webapi.api.v1.rest.mdp.edgebanding;
...
@Tag(name = "EdgeBandings")
@RestController
@RequestMapping(value = "/edgeBandings")
public class EdgeBandingController {

    private final EdgeBandingService service;
    private final EdgeBandingRepository repository;

    public EdgeBandingController(EdgeBandingService service, EdgeBandingRepository repository) {
    ...
    public ResponseEntity<?> findByStatusInAndDescriptionContainingIgnoreCase(@RequestParam(value = "status", required = false) String statusParam,
        ...
        return ResponseEntity.ok().body(repository.findByStatusInAndDescriptionContainingIgnoreCase(statusList, description, pageable, EdgeBandingDTO.class));
    }
    ...
    public ResponseEntity<List<DEdgeBanding>> findAllActiveAndCurrentOne(@RequestParam(value = "id", required = false) Long id){
        List<DEdgeBanding> list = service.findAllActiveAndCurrentOne(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_ANALYST', 'ROLE_OPERATOR')")
    @GetMapping(value = "/{id}")
    public ResponseEntity<DEdgeBanding> findById(@PathVariable Long id){
        DEdgeBanding dto = service.find(id);
    ...
    @PostMapping
    public ResponseEntity<DEdgeBanding> insert(@Valid @RequestBody DEdgeBanding dto){
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_ANALYST')")
    @PutMapping(value = "/{id}")
    public ResponseEntity<DEdgeBanding> update(@PathVariable Long id, @Valid @RequestBody DEdgeBanding dto){
    ...
    public ResponseEntity<DEdgeBanding> inactivate(@PathVariable Long id){
    ...
    public ResponseEntity<DEdgeBanding> delete(@PathVariable Long id){
        service.delete(id);
