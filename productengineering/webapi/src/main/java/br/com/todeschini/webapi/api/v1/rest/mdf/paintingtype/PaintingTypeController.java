package br.com.todeschini.webapi.api.v1.rest.mdf.paintingtype;
...
@Tag(name = "PaintingTypes")
@RestController
@RequestMapping(value = "/paintingTypes")
public class PaintingTypeController {
    ...
    public PaintingTypeController(PaintingTypeService service, PaintingTypeRepository repository) {
    ...
    public ResponseEntity<?> findByStatusInAndDescriptionContainingIgnoreCase(@RequestParam(value = "status", required = false) String statusParam,
        ...
        return ResponseEntity.ok().body(repository.findByStatusInAndDescriptionContainingIgnoreCase(statusList, description, pageable, PaintingTypeDTO.class));
    }
    ...
    public ResponseEntity<List<DPaintingType>> findAllActiveAndCurrentOne(@RequestParam(value = "id", required = false) Long id){
        List<DPaintingType> list = service.findAllActiveAndCurrentOne(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @GetMapping(value = "/{id}")
    public ResponseEntity<DPaintingType> findById(@PathVariable Long id){
        DPaintingType dto = service.find(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @PostMapping
    public ResponseEntity<DPaintingType> insert(@Valid @RequestBody DPaintingType dto){
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @PutMapping(value = "/{id}")
    public ResponseEntity<DPaintingType> update(@PathVariable Long id, @Valid @RequestBody DPaintingType dto){
    ...
    public ResponseEntity<DPaintingType> inactivate(@PathVariable Long id){
    ...
    public ResponseEntity<DPaintingType> delete(@PathVariable Long id){
        service.delete(id);
