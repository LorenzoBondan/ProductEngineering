package br.com.todeschini.webapi.api.v1.rest.aluminium.usedtrysquare;
...
import br.com.todeschini.webapi.api.v1.rest.aluminium.usedtrysquare.projection.UsedTrySquareDTO;
...
@Tag(name = "UsedTrySquares")
@RestController
@RequestMapping(value = "/usedTrySquares")
public class UsedTrySquareController {

    private final UsedTrySquareService service;
    private final UsedTrySquareRepository repository;

    public UsedTrySquareController(UsedTrySquareService service, UsedTrySquareRepository repository) {
    ...
    public ResponseEntity<?> findByStatusIn(@RequestParam(value = "status", required = false) String statusParam,
        ...
        return ResponseEntity.ok().body(repository.findByStatusIn(statusList, pageable, UsedTrySquareDTO.class));
    }
    ...
    public ResponseEntity<List<DUsedTrySquare>> findAllActiveAndCurrentOne(@RequestParam(value = "id", required = false) Long id){
        List<DUsedTrySquare> list = service.findAllActiveAndCurrentOne(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @GetMapping(value = "/{id}")
    public ResponseEntity<DUsedTrySquare> findById(@PathVariable Long id){
        DUsedTrySquare dto = service.find(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @PostMapping
    public ResponseEntity<DUsedTrySquare> insert(@Valid @RequestBody DUsedTrySquare dto){
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @PutMapping(value = "/{id}")
    public ResponseEntity<DUsedTrySquare> update(@PathVariable Long id, @Valid @RequestBody DUsedTrySquare dto){
    ...
    public ResponseEntity<DUsedTrySquare> inactivate(@PathVariable Long id){
    ...
    public ResponseEntity<DUsedTrySquare> delete(@PathVariable Long id){
        service.delete(id);
