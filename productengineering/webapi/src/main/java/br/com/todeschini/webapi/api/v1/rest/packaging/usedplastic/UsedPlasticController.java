package br.com.todeschini.webapi.api.v1.rest.packaging.usedplastic;
...
@Tag(name = "UsedPlastics")
@RestController
@RequestMapping(value = "/usedPlastics")
public class UsedPlasticController {

    private final UsedPlasticService service;
    private final UsedPlasticRepository repository;

    public UsedPlasticController(UsedPlasticService service, UsedPlasticRepository repository) {
    ...
    public ResponseEntity<?> findByStatusIn(@RequestParam(value = "status", required = false) String statusParam,
        ...
        return ResponseEntity.ok().body(repository.findByStatusIn(statusList, pageable, UsedPlasticDTO.class));
    }
    ...
    public ResponseEntity<List<DUsedPlastic>> findAllActiveAndCurrentOne(@RequestParam(value = "id", required = false) Long id){
        List<DUsedPlastic> list = service.findAllActiveAndCurrentOne(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_ANALYST', 'ROLE_OPERATOR')")
    @GetMapping(value = "/{id}")
    public ResponseEntity<DUsedPlastic> findById(@PathVariable Long id){
        DUsedPlastic dto = service.find(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_ANALYST', 'ROLE_OPERATOR')")
    @PostMapping
    public ResponseEntity<DUsedPlastic> insert(@Valid @RequestBody DUsedPlastic dto){
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_ANALYST', 'ROLE_OPERATOR')")
    @PutMapping(value = "/{id}")
    public ResponseEntity<DUsedPlastic> update(@PathVariable Long id, @Valid @RequestBody DUsedPlastic dto){
    ...
    public ResponseEntity<DUsedPlastic> inactivate(@PathVariable Long id){
    ...
    public ResponseEntity<DUsedPlastic> delete(@PathVariable Long id){
        service.delete(id);
