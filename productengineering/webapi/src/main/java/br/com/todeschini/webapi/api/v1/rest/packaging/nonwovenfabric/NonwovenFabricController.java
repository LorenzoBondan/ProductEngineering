package br.com.todeschini.webapi.api.v1.rest.packaging.nonwovenfabric;
...
@Tag(name = "NonwovenFabrics")
@RestController
@RequestMapping(value = "/nonwovenFabrics")
public class NonwovenFabricController {

    private final NonwovenFabricService service;
    private final NonwovenFabricRepository repository;

    public NonwovenFabricController(NonwovenFabricService service, NonwovenFabricRepository repository) {
    ...
    public ResponseEntity<?> findByStatusInAndDescriptionContainingIgnoreCase(@RequestParam(value = "status", required = false) String statusParam,
        ...
        return ResponseEntity.ok().body(repository.findByStatusInAndDescriptionContainingIgnoreCase(statusList, description, pageable, NonwovenFabricDTO.class));
    }
    ...
    public ResponseEntity<List<DNonwovenFabric>> findAllActiveAndCurrentOne(@RequestParam(value = "id", required = false) Long id){
        List<DNonwovenFabric> list = service.findAllActiveAndCurrentOne(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_ANALYST', 'ROLE_OPERATOR')")
    @GetMapping(value = "/{id}")
    public ResponseEntity<DNonwovenFabric> findById(@PathVariable Long id){
        DNonwovenFabric dto = service.find(id);
    ...
    @PostMapping
    public ResponseEntity<DNonwovenFabric> insert(@Valid @RequestBody DNonwovenFabric dto){
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_ANALYST')")
    @PutMapping(value = "/{id}")
    public ResponseEntity<DNonwovenFabric> update(@PathVariable Long id, @Valid @RequestBody DNonwovenFabric dto){
    ...
    public ResponseEntity<DNonwovenFabric> inactivate(@PathVariable Long id){
    ...
    public ResponseEntity<DNonwovenFabric> delete(@PathVariable Long id){
        service.delete(id);
