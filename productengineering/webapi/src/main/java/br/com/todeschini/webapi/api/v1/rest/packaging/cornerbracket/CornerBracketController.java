package br.com.todeschini.webapi.api.v1.rest.packaging.cornerbracket;
...
@Tag(name = "CornerBrackets")
@RestController
@RequestMapping(value = "/cornerBrackets")
public class CornerBracketController {

    private final CornerBracketService service;
    private final CornerBracketRepository repository;

    public CornerBracketController(CornerBracketService service, CornerBracketRepository repository) {
    ...
    public ResponseEntity<?> findByStatusInAndDescriptionContainingIgnoreCase(@RequestParam(value = "status", required = false) String statusParam,
        ...
        return ResponseEntity.ok().body(repository.findByStatusInAndDescriptionContainingIgnoreCase(statusList, description, pageable, CornerBracketDTO.class));
    }
    ...
    public ResponseEntity<List<DCornerBracket>> findAllActiveAndCurrentOne(@RequestParam(value = "id", required = false) Long id){
        List<DCornerBracket> list = service.findAllActiveAndCurrentOne(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @GetMapping(value = "/{id}")
    public ResponseEntity<DCornerBracket> findById(@PathVariable Long id){
        DCornerBracket dto = service.find(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @PostMapping
    public ResponseEntity<DCornerBracket> insert(@Valid @RequestBody DCornerBracket dto){
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @PutMapping(value = "/{id}")
    public ResponseEntity<DCornerBracket> update(@PathVariable Long id, @Valid @RequestBody DCornerBracket dto){
    ...
    public ResponseEntity<DCornerBracket> inactivate(@PathVariable Long id){
    ...
    public ResponseEntity<DCornerBracket> delete(@PathVariable Long id){
        service.delete(id);
