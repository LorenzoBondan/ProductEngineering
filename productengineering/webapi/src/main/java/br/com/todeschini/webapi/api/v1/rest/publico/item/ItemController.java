package br.com.todeschini.webapi.api.v1.rest.publico.item;
...
import br.com.todeschini.webapi.api.v1.rest.publico.item.projection.ItemDTO;
...
public class ItemController {
    ...
    public ResponseEntity<?> findByStatusInAndNameContainingIgnoreCase(@RequestBody(required = false) List<Status> statusList,
        ...
        return ResponseEntity.ok().body(repository.findByStatusInAndDescriptionContainingIgnoreCase(statusList, description, pageable, ItemDTO.class));
    }
    ...
    public ResponseEntity<List<DItem>> findAllActiveAndCurrentOne(@RequestParam(value = "id", required = false) Long id){
        List<DItem> list = service.findAllActiveAndCurrentOne(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @GetMapping(value = "/{id}")
    public ResponseEntity<DItem> findById(@PathVariable Long id){
        DItem dto = service.find(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @PostMapping
    public ResponseEntity<DItem> insert(@Valid @RequestBody DItem dto){
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @PutMapping(value = "/{id}")
    public ResponseEntity<DItem> update(@PathVariable Long id, @Valid @RequestBody DItem dto){
    ...
    public ResponseEntity<DItem> inactivate(@PathVariable Long id){
    ...
    public ResponseEntity<DItem> delete(@PathVariable Long id){
        service.delete(id);
