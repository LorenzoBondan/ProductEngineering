package br.com.todeschini.webapi.api.v1.rest.publico.son;
...
public class SonController {
    ...
    private final SonService service;
    private final SonRepository repository;

    public SonController(SonService service, SonRepository repository) {
    ...
    public ResponseEntity<?> findByStatusInAndDescriptionContainingIgnoreCase(@RequestParam(value = "status", required = false) String statusParam,
        ...
        return ResponseEntity.ok().body(repository.findByStatusInAndDescriptionContainingIgnoreCase(statusList, description, pageable, SonDTO.class));
    }
    ...
    public ResponseEntity<List<DSon>> findAllActiveAndCurrentOne(@RequestParam(value = "id", required = false) Long id){
        List<DSon> list = service.findAllActiveAndCurrentOne(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @GetMapping(value = "/{id}")
    public ResponseEntity<DSon> findById(@PathVariable Long id){
        DSon dto = service.find(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @PostMapping
    public ResponseEntity<DSon> insert(@Valid @RequestBody DSon dto){
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @PutMapping(value = "/{id}")
    public ResponseEntity<DSon> update(@PathVariable Long id, @Valid @RequestBody DSon dto){
    ...
    public ResponseEntity<DSon> inactivate(@PathVariable Long id){
    ...
    public ResponseEntity<DSon> delete(@PathVariable Long id){
        service.delete(id);
