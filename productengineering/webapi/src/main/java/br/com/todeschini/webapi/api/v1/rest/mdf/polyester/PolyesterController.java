package br.com.todeschini.webapi.api.v1.rest.mdf.polyester;
...
@Tag(name = "Polyesters")
@RestController
@RequestMapping(value = "/polyesters")
public class PolyesterController {

    private final PolyesterService service;
    private final PolyesterRepository repository;

    public PolyesterController(PolyesterService service, PolyesterRepository repository) {
    ...
    public ResponseEntity<?> findByStatusInAndDescriptionContainingIgnoreCase(@RequestBody(required = false) List<Status> statusList,
        ...
        return ResponseEntity.ok().body(repository.findByStatusInAndDescriptionContainingIgnoreCase(statusList, description, pageable, PolyesterDTO.class));
    }
    ...
    public ResponseEntity<List<DPolyester>> findAllActiveAndCurrentOne(@RequestParam(value = "id", required = false) Long id){
        List<DPolyester> list = service.findAllActiveAndCurrentOne(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @GetMapping(value = "/{id}")
    public ResponseEntity<DPolyester> findById(@PathVariable Long id){
        DPolyester dto = service.find(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @PostMapping
    public ResponseEntity<DPolyester> insert(@Valid @RequestBody DPolyester dto){
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @PutMapping(value = "/{id}")
    public ResponseEntity<DPolyester> update(@PathVariable Long id, @Valid @RequestBody DPolyester dto){
    ...
    public ResponseEntity<DPolyester> inactivate(@PathVariable Long id){
    ...
    public ResponseEntity<DPolyester> delete(@PathVariable Long id){
        service.delete(id);
