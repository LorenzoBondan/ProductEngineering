package br.com.todeschini.webapi.api.v1.rest.aluminium.usedmolding;
...
import br.com.todeschini.webapi.api.v1.rest.aluminium.usedmolding.projection.UsedMoldingDTO;
...
@Tag(name = "UsedMoldings")
@RestController
@RequestMapping(value = "/usedMoldings")
public class UsedMoldingController {

    private final UsedMoldingService service;
    private final UsedMoldingRepository repository;

    public UsedMoldingController(UsedMoldingService service, UsedMoldingRepository repository) {
    ...
    public ResponseEntity<?> findByStatusIn(@RequestParam(value = "status", required = false) String statusParam,
        ...
        return ResponseEntity.ok().body(repository.findByStatusIn(statusList, pageable, UsedMoldingDTO.class));
    }
    ...
    public ResponseEntity<List<DUsedMolding>> findAllActiveAndCurrentOne(@RequestParam(value = "id", required = false) Long id){
        List<DUsedMolding> list = service.findAllActiveAndCurrentOne(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_ANALYST', 'ROLE_OPERATOR')")
    @GetMapping(value = "/{id}")
    public ResponseEntity<DUsedMolding> findById(@PathVariable Long id){
        DUsedMolding dto = service.find(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_ANALYST', 'ROLE_OPERATOR')")
    @PostMapping
    public ResponseEntity<DUsedMolding> insert(@Valid @RequestBody DUsedMolding dto){
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_ANALYST', 'ROLE_OPERATOR')")
    @PutMapping(value = "/{id}")
    public ResponseEntity<DUsedMolding> update(@PathVariable Long id, @Valid @RequestBody DUsedMolding dto){
    ...
    public ResponseEntity<DUsedMolding> inactivate(@PathVariable Long id){
    ...
    public ResponseEntity<DUsedMolding> delete(@PathVariable Long id){
        service.delete(id);
