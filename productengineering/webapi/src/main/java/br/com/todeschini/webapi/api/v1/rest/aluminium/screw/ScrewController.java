package br.com.todeschini.webapi.api.v1.rest.aluminium.screw;
...
@Tag(name = "Screws")
@RestController
@RequestMapping(value = "/screws")
public class ScrewController {

    private final ScrewService service;
    private final ScrewRepository repository;

    public ScrewController(ScrewService service, ScrewRepository repository) {
    ...
    public ResponseEntity<?> findByStatusInAndDescriptionContainingIgnoreCase(@RequestParam(value = "status", required = false) String statusParam,
        ...
        return ResponseEntity.ok().body(repository.findByStatusInAndDescriptionContainingIgnoreCase(statusList, description, pageable, ScrewDTO.class));
    }
    ...
    public ResponseEntity<List<DScrew>> findAllActiveAndCurrentOne(@RequestParam(value = "id", required = false) Long id){
        List<DScrew> list = service.findAllActiveAndCurrentOne(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @GetMapping(value = "/{id}")
    public ResponseEntity<DScrew> findById(@PathVariable Long id){
        DScrew dto = service.find(id);
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @PostMapping
    public ResponseEntity<DScrew> insert(@Valid @RequestBody DScrew dto){
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_CLIENT')")
    @PutMapping(value = "/{id}")
    public ResponseEntity<DScrew> update(@PathVariable Long id, @Valid @RequestBody DScrew dto){
    ...
    public ResponseEntity<DScrew> inactivate(@PathVariable Long id){
    ...
    public ResponseEntity<DScrew> delete(@PathVariable Long id){
        service.delete(id);
